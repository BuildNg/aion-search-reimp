...
| `spec_probes/paired.py` | paired image-only, spectrum-only, and fusion spec-z comparison plus the shared paired readout used by the original run and cached sensitivity | data downloads, model-specific inference |
| `spec_probes/morphology_coverage.py` | coordinate matching to the public Galaxy Zoo DESI catalog, decision-tree-aware reliable-label flags, and redshift support tables | catalog download, model predictions, retrieval scoring |

After the HSC crossmatch gate, the paired experiment remains inside `spec_probes` because it uses the same frozen encoder and physical-label machinery. The full 1.126M-row HATS catalog already carries the spectrum arrays, so CPU preparation maps the 1,576 locked coordinates to adaptive HATS partitions and downloads only target-bearing partitions. Each completed partition is cached atomically and reused after interruption. A lightweight preflight resolves every target to a partition, reads one real matched spectrum, and round-trips its cache. The GPU command embeds the completed cache once and compares median, image-only, spectrum-only, and image+spectrum conditions under identical object splits and ridge fitting. It does not add a second split implementation, metric implementation, encoder adapter, or config framework.

`scripts/run_phase6_prechecks.py` owns the two CPU-only checks that follow the paired result. `alpha-sensitivity` reloads the saved image and spectrum embeddings, verifies their recorded fingerprints, and refits the same heads with the wider configured alpha grid; it never reloads either encoder. `galaxy-zoo` reads only coordinates and required vote-fraction columns from the checksum-pinned public Galaxy Zoo DESI friendly catalog, matches the locked DESI coordinates within the configured radius, and reports full-population denominators, match fractions, and label support by redshift bin. A Galaxy Zoo ID assigned to more than one paired object stops the audit before counting. Child morphology flags pass only when their decision-tree parents and their own vote fraction all exceed the configured confidence threshold. Neither check reads retrieval results or changes the paired population.
...
The locked headline queries are featured/disk and spiral at `0.05 <= z < 0.15`, with 107 and 78 positives respectively in the full labelled benchmark. nDCG@10 uses Galaxy Zoo path strength inside the interval; Recall@10 uses the reliable 0.7 morphology flag and the same hard redshift interval. Row-level rankings retain selection reason. Tables report the enriched benchmark, anchor, and morphology-priority strata separately; only the enriched benchmark is the powered pilot, and none estimates HSC-population performance.
